...
module cpu(
    ...
    // -------------------------------------------------------------------------
    // Memory Interface (Serialized)
    // -------------------------------------------------------------------------
    reg [3:0]  m_state; // 0: IDLE, 1-4: FETCH, 5-8: LOAD, 9-12: STORE
    reg [31:0] m_addr;
    reg [31:0] m_data_in;
    reg [31:0] m_data_out;
    reg [2:0]  m_size;
    ...
    // Execution State
    // -------------------------------------------------------------------------
    reg [3:0] state; // 0: IF, 1: ID/EX, 2: MEM, 3: WB
        ...
        if (rst_in) begin
            ...
            case (state)
                ...
                1: begin // ID/EX
                    ...
                    case (opcode)
                        7'h37: begin // LUI
                            if (rd != 0) regs[rd] <= {inst[31:12], 12'b0};
                            pc <= pc + 4; state <= 0;
                        end
                        7'h17: begin // AUIPC
                            if (rd != 0) regs[rd] <= pc + {inst[31:12], 12'b0};
                            pc <= pc + 4; state <= 0;
                        end
                        7'h6F: begin // JAL
                        ...
                        7'h03: begin // LOAD
                            imm = {{20{inst[31]}}, inst[31:20]};
                            m_addr <= rv1 + imm; m_wr <= 0; m_state <= 5;
                            state <= 2;
                        end
                        7'h23: begin // STORE
                            imm = {{20{inst[31]}}, inst[31:25], inst[11:7]};
                            m_addr <= rv1 + imm; m_data_out <= rv2; m_wr <= 1; m_state <= 9;
                            state <= 3;
                        end
                        7'h13: begin // OP-IMM
                            imm = {{20{inst[31]}}, inst[31:20]};
                ...
                2: begin // LOAD MEM
                    case (funct3)
                        3'h0, 3'h4: begin // LB, LBU
                            if (m_state == 5) begin
                                if (rd != 0) regs[rd] <= (funct3 == 3'h0) ? {{24{mem_din[7]}}, mem_din} : {24'b0, mem_din};
                                m_state <= 0; state <= 0; pc <= pc + 4;
                            end
                        end
                        3'h1, 3'h5: begin // LH, LHU
                            if (m_state == 6) begin
                                rv1 = {mem_din, m_data_in[7:0]};
                                if (rd != 0) regs[rd] <= (funct3 == 3'h1) ? {{16{rv1[15]}}, rv1[15:0]} : {16'b0, rv1[15:0]};
                                m_state <= 0; state <= 0; pc <= pc + 4;
                            end else begin
                                m_data_in[7:0] <= mem_din;
                                m_addr <= m_addr + 1; m_state <= m_state + 1;
                            end
                        end
                        3'h2: begin // LW
                            if (m_state == 8) begin
                                if (rd != 0) regs[rd] <= {mem_din, m_data_in[23:0]};
                                m_state <= 0; state <= 0; pc <= pc + 4;
                            end else begin
                        ...
                        default: begin m_state <= 0; state <= 0; pc <= pc + 4; end
                    endcase
                end
